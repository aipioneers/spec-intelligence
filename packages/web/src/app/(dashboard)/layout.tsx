...
function UserMenu() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex h-8 w-8 items-center justify-center rounded-full bg-accent-3 text-sm font-medium text-accent-11 hover:bg-accent-4 focus:outline-none focus:ring-2 focus:ring-accent-8"
      ...
      {open && (
        <>
          ...
          />
          {/* Dropdown */}
          <div className="absolute right-0 z-40 mt-2 w-48 rounded-md border border-gray-6 bg-gray-2 py-1 shadow-lg">
            <a
              href="/settings"
              className="block px-4 py-2 text-sm text-gray-11 hover:bg-gray-2"
              onClick={() => setOpen(false)}
            >
              Settings
            </a>
            <button
              ...
              className="block w-full px-4 py-2 text-left text-sm text-gray-11 hover:bg-gray-2"
            >
              Sign out
            </button>
          </div>
...
// ---------------------------------------------------------------------------
// Project switcher (bottom of sidebar)
// ---------------------------------------------------------------------------
...
function ProjectSwitcher({ collapsed }: { collapsed: boolean }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setProjectName(saved.split("/").filter(Boolean).pop() ?? saved);
    }
  }, []);
  ...
  return (
    <div className="border-t border-gray-6 px-2 py-3">
      <button
        onClick={handleSwitch}
        className={`flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-sm transition-colors text-gray-9 hover:bg-gray-3 hover:text-gray-11 ${
          collapsed ? "justify-center" : ""
        }`}
...
export default function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-gray-1">
      ...
      {/* ---- Sidebar ---- */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-gray-6 bg-gray-2 transition-transform duration-200 lg:static lg:translate-x-0 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } ${sidebarCollapsed ? "w-16" : "w-60"}`}
      >
        {/* Logo */}
        <div className="flex h-14 items-center justify-between border-b border-gray-6 px-4">
          {!sidebarCollapsed && (
            <a href="/features" className="text-lg font-bold text-gray-12">
              Spec Intelligence
            </a>
          )}
          <button
            onClick={() => setSidebarCollapsed((v) => !v)}
            className="hidden rounded p-1 text-gray-9 hover:bg-gray-3 hover:text-gray-11 lg:block"
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="rounded p-1 text-gray-9 hover:bg-gray-3 hover:text-gray-11 lg:hidden"
      ...
      <div className="flex flex-1 flex-col">
        {/* Top bar */}
        <header className="flex h-14 items-center justify-between border-b border-gray-6 bg-gray-2 px-4">
          <div className="flex items-center gap-3">
            {/* Mobile hamburger */}
            <button
              onClick={() => setSidebarOpen(true)}
              className="rounded p-1.5 text-gray-9 hover:bg-gray-3 hover:text-gray-11 lg:hidden"
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="w-64 rounded-md border border-gray-6 bg-gray-2 py-1.5 pl-8 pr-3 text-sm text-gray-12 placeholder:text-gray-8 focus:border-accent-8 focus:bg-gray-2 focus:outline-none focus:ring-1 focus:ring-accent-8"
              />
            </div>
          </div>
